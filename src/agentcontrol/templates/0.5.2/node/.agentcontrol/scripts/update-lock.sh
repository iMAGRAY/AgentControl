...
if [[ -x "$SDK_ROOT/.venv/bin/python" ]]; then
  SBOM_PY="$SDK_ROOT/.venv/bin/python"
elif [[ -x "$SDK_ROOT/.agentcontrol/.venv/bin/python" ]]; then
  SBOM_PY="$SDK_ROOT/.agentcontrol/.venv/bin/python"
else
  sdk::die "update-lock: .venv environment not found — run agentcall setup"
fi

sdk::log "INF" "Generating SBOM $SBOM_DST"
"$SBOM_PY" "$SCRIPT_DIR/generate-sbom.py" --output "$SBOM_DST"
