...
def generate_fallback_response(task_id: str, prompt_path: Path, command: str) -> str:
    ...
    lines.append("    \"2. Prioritise open blockers in tasks.board.json\",")
    lines.append("    \"3. Update architecture_plan.md with latest outcomes\"")
    lines.append("  ]")
    lines.append("}")
    return "\n".join(lines)
...
def main(argv: List[str] | None = None) -> int:
    ...
    if not command_template:
        ...
        stdout = generate_fallback_response(args.task, prompt_path, args.command)
