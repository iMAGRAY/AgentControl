...
def writable_dir(candidate: Path) -> Path | None:
    try:
        ...
        print(
            f"[agents-auth] no permissions for directory {candidate} — seeking alternative",
            file=sys.stderr,
        )
...
def resolve_state_dir() -> Path:
    ...
    if fallback is not None:
        return fallback
    raise PermissionError("unable to locate writable directory for state storage")
    ...
    for src in targets:
        ...
        try:
            if agent_state_dir in src_path.parents:
                warnings.append(f"[agents-auth] {agent}: skipping copy {src_path} (already within state)")
                continue
            if src_path.is_dir():
    ...
    for raw in paths:
        ...
        try:
            if candidate.is_file():
                return True
            if candidate.is_dir():
                # Check for at least one file inside the directory
