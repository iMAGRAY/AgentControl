#!/usr/bin/env python3
"""Architecture manifest tooling for AgentControl Universal Agent SDK."""
...
DOCS_ROOT = Path("agentcontrol") / "docs"


def _doc_path(*parts: str) -> str:
    return DOCS_ROOT.joinpath(*parts).as_posix()
...
def render_architecture_overview(manifest: dict) -> str:
    ...
    lines.extend([
        "",
        "## Documents",
        f"- ADR Index: {_doc_path('adr', 'index.md')}",
        f"- RFC Index: {_doc_path('rfc', 'index.md')}",
...
def render_adr_files(manifest: dict) -> Dict[str, str]:
    ...
    for adr in adr_entries:
        ...
        outputs[_doc_path("adr", f"{adr['id']}.md")] = content
    outputs[_doc_path("adr", "index.md")] = "\n".join(lines) + "\n"
    return outputs
...
def render_rfc_files(manifest: dict) -> Dict[str, str]:
    ...
    for rfc in rfc_entries:
        ...
        outputs[_doc_path("rfc", f"{rfc['id']}.md")] = content
    outputs[_doc_path("rfc", "index.md")] = "\n".join(lines) + "\n"
    return outputs
...
def generate_outputs(manifest: dict) -> Dict[str, str]:
    ...
    outputs[_doc_path("architecture", "overview.md")] = render_architecture_overview(manifest)
