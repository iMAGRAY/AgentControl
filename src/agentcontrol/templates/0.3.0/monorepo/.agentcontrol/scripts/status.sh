...
project_root = Path(os.environ["AGENTCONTROL_PROJECT_ROOT"])
service = DocsBridgeService()
try:
    docs_bridge = service.diagnose(project_root)
except Exception as exc:  # pragma: no cover - resilience for shell pipeline
    docs_bridge = {
        "status": "error",
        "issues": [
            {
                "severity": "error",
                "code": "DOC_BRIDGE_STATUS_FAILURE",
                "message": str(exc),
                "remediation": "Run `agentcall docs diagnose --json` to inspect details.",
            }
        ],
    }
