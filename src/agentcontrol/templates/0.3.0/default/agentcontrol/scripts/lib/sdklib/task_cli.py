...
Redesigned task.sh implementation: adds locking, caching,
...
except ModuleNotFoundError:  # pragma: no cover - expected on non-POSIX
...
def global_lock(*, exclusive: bool) -> Iterable[None]:
    ...
    if fcntl is None:  # pragma: no cover - fallback without locking
        yield
        return
...
class TaskSession:
    """Task board snapshot within locked context."""
...
def validate_board(session: TaskSession) -> None:
    ...
    for task in session.board.get("tasks", []):
        ...
        if task.get("status") == "blocked" and not task.get("blockers"):
            raise SystemExit(f"Task {task_id} marked blocked without blockers")
...
def assign_task(session: TaskSession, task_id: str, agent: str, note: str, *, action: str, force: bool) -> None:
    ...
    for dep_id in task.get("dependencies", []):
        ...
        if dep.get("status") not in {"done", "review"} and session.assignments.get(dep_id) not in {agent} and not force:
            raise SystemExit(f"Dependency {dep_id} not yet ready (status {dep.get('status')})")
...
def grab_task(session: TaskSession, agent: str, note: str, *, force: bool) -> None:
    ...
    for task in ordered:
        ...
        return
    print("No available tasks to capture")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    list_parser = sub.add_parser("list", help="Show task board")
    list_parser.add_argument("--compact", action="store_true")

    status_parser = sub.add_parser("status", help="Alias for list")
    ...
    sub.add_parser("conflicts", help="Conflict map")
