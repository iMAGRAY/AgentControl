...
- Docs bridge: `.agentcontrol/config/docs.bridge.yaml` фиксирует, какие файлы документации патчит автоматическая синхронизация.
- Mission control & digital twin (roadmap): `agentcall mission --json` читает `.agentcontrol/state/twin.json`, отдавая агенту единый снимок состояния (docs/tests/tasks/MCP).
