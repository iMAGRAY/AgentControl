...
def test_workspace_summary_json(capsys: pytest.CaptureFixture[str]) -> None:
    exit_code = cli_main.main(["mission", "summary", "--workspace", "--json"])
    assert exit_code == 0
    output = capsys.readouterr().out
    summary = json.loads(output)
    assert "projects" in summary
    assert any(proj["id"] == "agentcontrol-sdk" for proj in summary["projects"])
