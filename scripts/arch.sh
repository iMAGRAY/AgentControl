...
usage() {
  cat <<USAGE
Использование: make arch-edit | make arch-apply | scripts/arch.sh <edit|apply>
USAGE
}
...
case "$command" in
  edit)
    if [[ -f "$EDIT_MANIFEST" ]]; then
      ...
      echo "Скопирован manifest.yaml → manifest.edit.yaml. Отредактируйте manifest.edit.yaml и выполните make arch-apply." >&2
    fi
    ;;
  apply)
    if [[ ! -f "$EDIT_MANIFEST" ]]; then
      echo "architecture/manifest.edit.yaml не найден. Выполните make arch-edit." >&2
      exit 1
    fi
    python3 - <<'PY'
