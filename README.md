...
## Быстрый старт
1. `make setup` — устанавливает системные утилиты (shellcheck, go и т.п.), разворачивает `.venv` и ставит Python/Go-зависимости (pytest, diff-cover, detect-secrets, reviewdog).
2. `make lock` — регенерирует `requirements.lock` и `sbom/python.json` через pip-compile (pip-tools) и фиксирует актуальные хеши.
3. `make verify` — выполняет проверки/тесты, сверяет окружение с lock/SBOM и запускает `pip-audit` (отчёт `reports/pip-audit.json`/`.sarif`). GitHub Actions гоняет эту цепочку на каждом push/PR и по ночному расписанию (03:00 UTC), выкладывая SARIF в Security Alerts.
