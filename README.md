...
Помимо make-таргетов доступен Python-CLI: `python3 scripts/sdk.py <command>`. Поддерживаются `verify`, `review [--base <commit>]`, `doctor`, `status` и `qa` (цепочка `verify` → `review`). Это снижает когнитивную нагрузку, если хочется управлять SDK без Make.
